...
export function SalesGaugeChart({ gaugeData = [], totalEnrollments = 0, totalRevenue = 0 }: { gaugeData?: any[], totalEnrollments?: number, totalRevenue?: number }) {
  // Use a fallback if there's no data
  const data = gaugeData.length > 0 ? gaugeData : [{ name: 'No Enrollments', value: 1 }];
  return (
    <div className="flex flex-col p-6 bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 rounded-3xl h-full min-h-[400px]">
      ...
      <div className="relative flex items-center justify-center flex-1 w-full">
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[20%] flex flex-col items-center text-center">
          <span className="text-3xl font-bold text-gray-900 dark:text-white max-w-[140px] truncate">{data[0]?.name?.substring(0, 4) || ''}</span>
          <span className="mt-1 text-xs text-gray-500 dark:text-gray-400">Top Course</span>
        </div>
      ...
      <div className="flex items-center gap-4 mt-6">
        <div className="flex flex-col flex-1 p-4 bg-gray-50 dark:bg-zinc-800/50 rounded-2xl">
          <span className="mb-2 text-sm font-medium text-gray-500 dark:text-gray-400">Total Enrollments</span>
          <div className="flex items-center gap-2">
            <span className="text-xl font-bold text-gray-900 dark:text-white">{totalEnrollments}</span>
        ...
        <div className="flex flex-col flex-1 p-4 bg-gray-50 dark:bg-zinc-800/50 rounded-2xl">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xl font-bold text-gray-900 dark:text-white">${Number(totalRevenue).toLocaleString()}</span>
